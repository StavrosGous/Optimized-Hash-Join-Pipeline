...
TEST_CASE("End Function of HopscotchMap", "[HashMap]") {
    HopscotchMap<int, std::vector<int>> map(10ll);
    REQUIRE(map.end() == nullptr);
    HopscotchMap<double, std::vector<int>> double_map(10ll);
    REQUIRE(double_map.end() == nullptr);
}

// Test case to test successful insertion and finding of keys
TEST_CASE("Insertion and Finding in HopscotchMap", "[HashMap]") {
    HopscotchMap<int, std::vector<int>> map(10ll);    
...
TEST_CASE("Finding Non-Existent Keys in HopscotchMap", "[HashMap]") {
    HopscotchMap<int, std::vector<int>> map(10ll);
    map.emplace(1, std::vector<int>{2, 3});
    map.emplace(2, std::vector<int>{4, 5});
    map.emplace(3, std::vector<int>{6, 7});
