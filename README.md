...
## Implementation
We followed as closely as possible the instructions of the assignment's description for the implementation of all algorithms. For optimization purposes we use [`crc_hash`](https://github.com/sigmod25-teamOPT/optimized-join-pipeline/blob/main/src/unchained_table.cpp) possible collisions in both `Robinhood` and `Hopscotch`. For the Cuckoo hashmap which needs two hash functions, we used [`crc_hash`]() as the first hash function and `std::hash` as the second.
