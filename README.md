...
## Implementation
We followed as closely as possible the instructions of the assignment's description for the implementation of all algorithms. For optimization purposes we use [`splitmix`](https://rosettacode.org/wiki/Pseudo-random_numbers/Splitmix64) after calling a hash function on keys to reduce possible collisions. For Robinhood and Hopscotch, we stuck with the `std::hash` function for Robinhood and Hopscotch. For the Cuckoo hashmap which needs two hash functions, we used std::hash as the first hash function and [`Murmurhash3`](https://medium.com/@thealonemusk/murmurhash-the-scrappy-algorithm-that-secretly-powers-half-the-internet-2d3f79b4509b) as the second.
